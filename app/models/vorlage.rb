class Vorlage < ActiveRecord::Base
  ...
  scope :in_beratung, lambda { joins(:sitzung).joins(:sitzung_vorlage).where("sitzung_vorlage.typ = 'Entscheidung' AND sitzung_vorlage.decission_id IS NULL AND sitzung.datum > ?",2.days.ago.beginning_of_day).order("sitzung.datum ASC").group("vorlage.id, sitzung.datum") }
